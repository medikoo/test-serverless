(function (global, factory) {
  ...
  var mainGroupTypes = new Set(["concat", "group", "indent"]);
  var lineBreakTypes = new Set(["line", "if-break"]);

  var groupLines = function groupLines(ind, MODE_FLAT, width, pos, doc, options, fits) {
    var contents = {
      type: "concat"
    };
    var next = [ind, MODE_FLAT, contents];
    var rem = width - pos;
    var groupedParts = [];
    var currentIndex = 0;
    var currentPart = doc.parts[currentIndex];
    var someGrouped = false;
    var groupedGroupsLength = 0;
    var currentItemStartIndex;
    var currentItemEndIndex;
    var currentGroupLastItemEndIndex;
    ...
    while (currentPart != null) {
      ...
      if (currentPart && mainGroupTypes.has(currentPart.type)) {
        ...
        while (doc.parts[currentIndex + 1] && !lineBreakTypes.has(doc.parts[currentIndex + 1].type)) {
          currentPart = doc.parts[++currentIndex];
        }

        currentItemEndIndex = currentIndex + 1;
        contents.parts = doc.parts.slice(currentGroupStartIndex, currentItemEndIndex);
        ...
        if (!fits(next, [], rem, options)) {
          if (currentGroupLastItemEndIndex) {
            ...
            if (groupedGroupsLength > 1) {
              ...
              someGrouped = true;
              groupedParts.push.apply(groupedParts, [// group of items
              {
                type: "group",
                break: false,
                contents: {
                  type: "concat",
                  parts: doc.parts.slice(currentGroupStartIndex, currentGroupLastItemEndIndex)
                }
              }].concat(_toConsumableArray(doc.parts.slice(currentGroupLastItemEndIndex, currentItemStartIndex))));
            } else {
              // Just one fits the line no need for extra groupping
              groupedParts.push.apply(groupedParts, _toConsumableArray(doc.parts.slice(currentGroupStartIndex, currentItemStartIndex)));
            }

            currentGroupStartIndex = currentItemStartIndex; // Reset index so we start again with group which didn't fit

            currentIndex = currentItemStartIndex - 1;
          } else {
            // Group on its own doesn't fit the line, pass it through
            groupedParts.push.apply(groupedParts, _toConsumableArray(doc.parts.slice(currentGroupStartIndex, currentItemEndIndex))); // Pass through any fruther coma or newline, until next group

            while (doc.parts[currentIndex + 1] && !mainGroupTypes.has(doc.parts[currentIndex + 1].type)) {
              currentPart = doc.parts[++currentIndex];
    ...
    if (someGrouped || groupedGroupsLength > 1) {
      ...
      if (groupedGroupsLength > 1) {
        ...
        groupedParts.push.apply(groupedParts, _toConsumableArray(doc.parts.slice(currentGroupStartIndex)));
      }
  ...
  var isLineBreaking = function isLineBreaking(doc, mode, MODE_BREAK, MODE_FLAT) {
    var remaining = [doc];
    ...
    while (cmds.length !== 0) {
      ...
      if (typeof _doc === "string") {
        ...
        switch (_doc.type) {
          ...
          case "concat":
            {
              var docParts = _doc.parts;

              if (_doc.groupLines && mode === MODE_BREAK && !docPrinter.isLineBreaking(_doc, mode, MODE_BREAK, MODE_FLAT)) {
                docParts = docPrinter.groupLines(ind, MODE_FLAT, width, pos, _doc, options, fits);
              }

              for (var i = docParts.length - 1; i >= 0; i--) {
