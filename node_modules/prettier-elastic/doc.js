(function (global, factory) {
  ...
  function printString(raw, options, isDirectiveLiteral) {
    ...
    const enclosingQuote = options.parser === "json" ? '"' : options.__isInHtmlAttribute ? "'" : getPreferredQuote(raw, options.singleQuote ? "'" : '"', options.parser); // Directives are exact code unit sequences, which means that you can't
  ...
  var groupLines = function groupLines({
    ind,
    MODE_FLAT,
    width,
    pos,
    ...
    while (currentPart != null) {
      ...
      if (currentPart && mainGroupTypes.has(currentPart.type)) {
        ...
        if (!fits(next, currentItemEndIndex === doc.parts.length ? cmds : [], rem, options)) {
          if (currentGroupLastItemEndIndex) {
            ...
            if (groupedGroupsLength > 1) {
              ...
              someGrouped = true;
              groupedParts.push( // group of items
              {
                type: "group",
                break: false,
                contents: {
              ...
              ...doc.parts.slice(currentGroupLastItemEndIndex, currentItemStartIndex));
            } else {
              // Just one fits the line no need for extra groupping
              groupedParts.push(...doc.parts.slice(currentGroupStartIndex, currentItemStartIndex));
            }
            ...
            groupedParts.push(...doc.parts.slice(currentGroupStartIndex, currentItemEndIndex)); // Pass through any fruther coma or newline, until next group
          ...
          case "concat":
            {
              ...
              if (doc.groupLines && mode === MODE_BREAK && !docPrinter.isLineBreaking(doc, mode, MODE_BREAK, MODE_FLAT)) {
                docParts = docPrinter.groupLines({
                  ind,
                  MODE_FLAT,
                  width,
                  pos,
